...
for _ in tqdm(range(num_samples)):
    ...
    random_dataid = np.random.choice(dataid_list, size=1, replace=False)
    ...
    for dataid in random_dataid:
        ...
        # Cancel row if nan
        dataid_data = dataid_data.dropna(axis=0, how='any')
        
        # Convert to numpy array and reshape
        reshaped_data = dataid_data.values.reshape(1, -1, 97)
        
        # Check if shape 1 large than 250
        if reshaped_data.shape[1] > 150:
            # Randomly select 250 data points
            random_indices = np.random.choice(reshaped_data.shape[1], size=250, replace=True)
...
# Save the new data as pickle file
with open(f'exp/data_process_for_data_collection_all/new_data_15minute_{data_type}_nomerge.pkl', 'wb') as f:
    pickle.dump(new_data, f)

with open(f'exp/data_process_for_data_collection_all/new_data_15minute_{data_type}_nomerge.pkl', 'rb') as f:
...
for i in range(20):
    ...
    plt.plot(new_data[i, :, :-1].T, color='blue', alpha=0.5)
    plt.title(f'Sample {i+1}')
    plt.xlabel('Time')
    plt.ylabel('Value')
    plt.legend()
plt.tight_layout()
plt.savefig(f'exp/data_process_for_data_collection_all/15minute_data_{data_type}_nomerge.png')
