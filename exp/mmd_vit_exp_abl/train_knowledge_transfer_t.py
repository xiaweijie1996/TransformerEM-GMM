...
import model.gmm_transformer_pos as gmm_model
...
save_model = 'mmd_vit_exp_abl/model/'
save_image = 'mmd_vit_exp_abl/gen_img/'
lr = 0.0001
n_components = 6
...
wandb.init(project=f'small_60m_{random_sample_num}_train', entity='xiaweijie1996')
...
for epoch in range(num_epochs):
    for j in range(sub_epoch):
        ...
        print('epoch: ', epoch, 'sub_epoch: ', j, 'loss_train: ', _loss.item(), 'random_num: ', _random_num)
    ...
    if epoch % 1 == 0:
        ...
        if _loss.item() < mid_loss:
            mid_loss = _loss.item()
            torch.save(encoder.state_dict(), save_model + f'small_60m_ktt_encoder_{random_sample_num}_{_model_scale}.pth')
            torch.save(embedding_para, save_model + f'small_60m_ktt_embedding_{random_sample_num}_{_model_scale}.pth')
            torch.save(emb_empty_token, save_model + f'small_60m_ktt_emb_empty_token_{random_sample_num}_{_model_scale}.pth')

    if epoch % 10 == 0:
        save_path = save_image+str(epoch)+f'small_60m_ktt_{random_sample_num}_{_model_scale}.png'
