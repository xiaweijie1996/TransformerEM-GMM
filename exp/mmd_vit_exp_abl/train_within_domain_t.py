...
save_model = '/home/weijiexia/paper3/mmd_vit_exp_abl/model/'
save_image = '/home/weijiexia/paper3/mmd_vit_exp_abl/gen_img/'
lr = 0.0001
n_components = 6
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        ...
        if _loss.item() < mid_loss:
            mid_loss = _loss.item()
            torch.save(encoder.state_dict(), save_model + f'small_60m_wdt_encoder_{random_sample_num}_{_model_scale}.pth')
            torch.save(embedding_para, save_model + f'small_60m_wdt_embedding_{random_sample_num}_{_model_scale}.pth')
            torch.save(emb_empty_token, save_model + f'small_60m_wdt_emb_empty_token_{random_sample_num}_{_model_scale}.pth')


    if epoch % 5 == 0:
        save_path = save_image+str(epoch)+f'small_60m_wdt_{random_sample_num}_{_model_scale}.png'
