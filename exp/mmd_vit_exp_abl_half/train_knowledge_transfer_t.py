...
wandb.init(project=f'small_{random_sample_num}_30m_ktt', entity='xiaweijie1996')
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        ...
        if _loss.item() < mid_loss:
            mid_loss = _loss.item()
            torch.save(encoder.state_dict(), save_model + f'small_30mr_encoder_ktt_{random_sample_num}_{_model_scale}.pth')
            torch.save(embedding_para, save_model + f'small_30mr_embedding_ktt_{random_sample_num}_{_model_scale}.pth')
            torch.save(emb_empty_token, save_model + f'small_30mr_emb_empty_token_ktt_{random_sample_num}_{_model_scale}.pth')

    if epoch % 10 == 0:
        save_path = save_image+str(epoch)+f'small_ktt_{random_sample_num}_{_model_scale}.png'
