...
def train_and_evaluate(model, data_loader, optimizer, device, epochs, sub_epoch, test_steps, num_params):
    ...
    for epoch in range(epochs):
        ...
        if loss < current_loss:
            current_loss = loss
            torch.save(model.state_dict(), f'exp/exp_second_round/timesnet_userload_mmd_15minutes/timesnet_MMD_{num_params}.pth')
        ...
        if epoch % 10 == 0:
            ...
            plt.legend(['x', 'y_hat'])
            plt.savefig(f'exp/exp_second_round/timesnet_userload_mmd_15minutes/timesnet_MMD_{num_params}.png')
            plt.close()
            wandb.log({'plot': wandb.Image(f'exp/exp_second_round/timesnet_userload_mmd_15minutes/timesnet_MMD_{num_params}.png')})
            # print('-------------------')