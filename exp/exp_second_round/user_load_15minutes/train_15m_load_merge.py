...
dataset.images = dataset.images + np.abs(np.random.normal(0.05, 0.05, dataset.images.shape)) 
...
save_image = 'exp/exp_second_round/user_load_15minutes/gen_img/'
lr = 0.0001
n_components = 6
...
for epoch in range(num_epochs):
    ...
    if epoch % 1 == 0:
        ...
        if _loss.item() < mid_loss:
            mid_loss = _loss.item()
            torch.save(encoder.state_dict(), save_model + f'loadmerge_encoder_{random_sample_num}_{_model_scale}.pth')
            torch.save(embedding_para, save_model + f'loadmerge_embedding_{random_sample_num}_{_model_scale}.pth')
            torch.save(emb_empty_token, save_model + f'loadmerge_emb_empty_token_{random_sample_num}_{_model_scale}.pth')
