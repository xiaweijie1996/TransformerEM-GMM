...
for _random_num in [4, 8, 16, 32]:
    ...
    for i in tqdm(range(batch_size)):
        ...
        samples = samples + np.random.rand(samples.shape[0], samples.shape[1]) * 0.01 # prevent the nan
        samples_gmm = samples_gmm + np.random.rand(samples_gmm.shape[0], samples_gmm.shape[1]) * 0.01 # prevent the nan
        ...
        _part_real = _part_real + np.random.rand(_part_real.shape[0], _part_real.shape[1]) * 0.01 # prevent the nan
