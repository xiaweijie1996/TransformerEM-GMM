...
for random_sample_num in [4, 8, 16, 32]:
    ...
    # load data
    # _sample_indx=[6, 37]
    test_sample = dataset.load_test_data(batch_size)  # (B,N,L)
    ...
    for i in tqdm(range(10)):
        ...
        kl += plot_eva.compute_kl_divergence(samples_real.cpu().detach().numpy(), samples_ddpm.cpu().detach().numpy())
        ks += ks_2samp(samples_real.flatten().cpu().detach().numpy(), samples_ddpm.flatten().cpu().detach().numpy())
        ...
        kl_flow += plot_eva.compute_kl_divergence(samples_real.detach().numpy, samples_flow.cpu().detach().numpy())
        ks_flow += ks_2samp(samples_real.flatten().cpu().detach().numpy(), samples_flow.flatten().cpu().detach().numpy())
