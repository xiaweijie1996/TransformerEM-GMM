...
def train_and_evaluate(model, data_loader, optimizer, device, epochs, sub_epoch, test_steps, num_params):
    current_loss =10000
    mmd_loss_function = MMDLoss(kernel_type='rbf', bandwidth=1.0)
    for epoch in range(epochs):
        ...
        mmd_loss = mmd_loss_function(train_data, y_hat)
        mse_loss = ((y_hat -train_data)**2) * index_mask.to(device)
        loss = mmd_loss * 3 + mse_loss.mean()
      
        wandb.log({'loss_train': loss.item(), 'mmd_loss': mmd_loss.item(), 'mse_loss': mse_loss.mean().item()})
        ...
        if loss < current_loss:
            current_loss = loss
            torch.save(model.state_dict(), f'exp/exp_second_round/timesnet_solar_mmd_15minutes/timesnet_MMD_{num_params}.pth')
        ...
        if epoch % 10 == 0:
            ...
            plt.legend(['x', 'y_hat'])
            plt.savefig(f'exp/exp_second_round/timesnet_solar_mmd_15minutes/timesnet_MMD_{num_params}.png')
            plt.close()
            wandb.log({'plot': wandb.Image(f'exp/exp_second_round/timesnet_solar_mmd_15minutes/timesnet_MMD_{num_params}.png')})
            # print('-------------------')