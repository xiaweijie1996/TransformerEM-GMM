...
def get_loss_le(dataset, encoder, random_sample_num, min_random_sample_num, n_components, embedding, emb_empty_token, train='True', device='cpu'):
    ...
    _covs = torch.ones(_train_sample_part.shape[0], n_components, 96).to(device)
...
def get_loss_parametertuning(dataset, encoder, random_sample_num, min_random_sample_num, n_components, embedding, emb_empty_token, train='True', device='cpu'):
    if train == 'True':
        train_sample = dataset.load_train_data()
    else:
        train_sample = dataset.load_test_data()
...
def get_loss_randomweight(dataset, encoder, random_sample_num, min_random_sample_num, n_components, embedding, emb_empty_token, weights, train='True', device='cpu'):
    if train == 'True':
        train_sample = dataset.load_train_data()
    else:
        train_sample = dataset.load_test_data()
    ...
    _new_para = encoder_out[:, :n_components*2, :]
    _weights = weights.repeat(train_sample.shape[0], 1)
    wandb.log({'weights_mean': _weights.mean().item(), 'weights_min': _weights.min().item(), 'weights_max': _weights.max().item()})
    # print('encoder out', encoder_out.shape, _new_para.shape, _weights.shape)
...
def get_loss_fullcov2(dataset, encoder, random_sample_num, min_random_sample_num, n_components, rank_level, embedding, emb_empty_token, train='True', device='cpu'):
    if train == 'True':
        train_sample = dataset.load_train_data()
    else:
        train_sample = dataset.load_test_data()
    ...
    d = 96
    K = n_components
    b = train_sample.shape[0]
    _new_para = encoder_out[:, :n_components*(rank_level+1), :]
    _new_para = encoder.output_adding_layer(_new_para, _param)
    _means = _new_para[:, :n_components, :]
    _covs = _new_para[:, n_components:, :].view(_new_para.shape[0], -1)
    _covs = _covs[:, :n_components*d*49].reshape(_new_para.shape[0], n_components, 97*48) # all the elements  of a 96 * 96 a lower triangular matrix 

    # 1Create index mask for lower-triangular positions
    idx = torch.tril_indices(d, d, 0, device=device)   # (2, d*(d+1)//2)
    # 2️Prepare output tensor
