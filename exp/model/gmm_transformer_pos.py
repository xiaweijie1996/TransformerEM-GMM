...
class ViT_encodernopara(nn.Module):
    ...
    def _add_positional_embedding(self, x):
        b, n, d = x.shape
        pos = torch.arange(0, n+1).to(x.device)
        pos_enc = torch.zeros(b, n+1, d).to(x.device)
        div_term = (1000**(torch.arange(0, n+1, 2).float()/d)).to(x.device)
        
        _len = len(pos[0::2])
        _pos_sin = torch.sin(pos[0::2].float() * div_term[:_len ]).unsqueeze(0).unsqueeze(-1)
        _pos_cos = torch.cos(pos[1::2].float() * div_term[:_len ]).unsqueeze(0).unsqueeze(-1)
        
        pos_enc[:, 0::2, :] =  _pos_sin.repeat(b, 1, d)
        pos_enc[:, 1::2, :] = _pos_cos.repeat(b, 1, d)
        return x + pos_enc[:, :-1, :]
        
    def forward(self, images):
        _images = images
        tokens = self.linear_map_in(_images)
        tokens = self._add_positional_embedding(tokens)
